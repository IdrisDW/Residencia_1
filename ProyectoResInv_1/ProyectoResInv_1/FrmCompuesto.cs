      ...
      //  MessageBox.Show(idS.ToString());
        ...
        private void left_Click(object sender, EventArgs e)
        {
            for (int i = dataGridViewRight.RowCount - 1; i >= 0; i--)
            {
                ...
                {
                    medicineBindingSource.Add((MedicineTableAdapter)row.DataBoundItem);
                        medicineBindingSource.RemoveAt(row.Index);
                }
        ...
        private void right_Click(object sender, EventArgs e)
        {
            for (int i = dataGridViewLeft.RowCount - 1; i >= 0; i--)
            {
                ...
                {
                    medicineBindingSource1.Add((MedicineTableAdapter)row.DataBoundItem);
                    medicineBindingSource.RemoveAt(row.Index);
                }
